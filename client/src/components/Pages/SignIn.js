...
export default function SignInSide() {
  ...
  const submitForm  = async (e) => {
    e.preventDefault();
    console.log(`[submitForm] passing in info: `,loginRef.current.value,passwordRef.current.value)

    // go do login to server
    let result = await axios.post('/api/login',
      {email: loginRef.current.value, password: passwordRef.current.value} );
    console.log( `~ login complete, data.status(${result.status}) result: `, result.data );
    if( result.data.status && result.data.status=="1" ){
      ...
      console.log( `sorry an error happened: ${result.data.error}` );
      localStorage.userID = "";
    }
  }
